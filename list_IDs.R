...
folder_path <- "S:/Meg_Stuff/FU3_nifti"  # <-- Replace with your actual path

# List all files in the folder (no need for full names)
files <- list.files(folder_path, full.names = FALSE)

# Remove extensions (.nii.gz, .nii) for all files
ids <- sub("\\.nii\\.gz$|\\.nii$", "", files)

# Remove any duplicates (to ensure each ID is listed only once, .nii.gz and .nii both present)
